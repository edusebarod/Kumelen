Public Class visRepEstadoCuenta
    Public tipo As Integer
    Public monto As Decimal
    Public cuenta As String
    Public fechadesde As String
    Public fechahasta As String
    Public objrep As New rptEstadoCuenta
    ...
    Private Sub visRepEstadoCuenta_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If tipo = 1 Then
            If fechadesde = fechahasta Then
                SQL = String.Format("(SELECT CONVERT(m.mov_fecha, CHAR) AS fecha, m.mov_concepto as concepto, m.mov_total as monto, m.mov_encargado as encargado, m.mov_obs as observaciones, m.mov_tipo as tipo, m.mov_id as id " +
                                   "FROM movimientos as m " +
                                   "WHERE (m.mov_cuentaDebe LIKE '{0}' OR m.mov_cuentaHaber LIKE '{0}') AND m.timestamp BETWEEN '{1} 00:00' AND '{2} 23:59') " +
                                   "UNION " +
                                   "(SELECT CONVERT(m.mov_fecha, CHAR) AS fecha, md.mde_concepto AS concepto, md.mde_monto AS monto, m.mov_encargado as encargado, m.mov_obs AS observaciones, m.mov_tipo as tipo, md.mde_id as id " +
            ...
            End Try
        ElseIf tipo = 2 Then
            SQL = String.Format("(SELECT m.mov_fecha as fecha, m.mov_concepto as concepto, m.mov_total as monto, m.mov_encargado as encargado, m.mov_obs as observaciones, m.mov_tipo as tipo, m.mov_id as id " +
                                "FROM movimientos as m " +
                                "WHERE (m.mov_cuentaDebe LIKE '{0}' OR m.mov_cuentaHaber LIKE '{0}') AND m.timestamp BETWEEN '{1}' AND '{2}') " +
                                "UNION " +
                                "(SELECT m.mov_fecha AS fecha, md.mde_concepto AS concepto, md.mde_monto AS monto, m.mov_encargado as encargado, m.mov_obs AS observaciones, m.mov_tipo as tipo, md.mde_id as id " +
                                "FROM movimientos AS m, movimientosdetalles AS md " +
        ...
        ElseIf tipo = 3 Then
            SQL = String.Format("(SELECT m.mov_fecha as fecha, m.mov_concepto as concepto, m.mov_total as monto, m.mov_encargado as encargado, m.mov_obs as observaciones, m.mov_tipo as tipo, m.mov_id as id " +
                                ...
                                "(SELECT m.mov_fecha AS fecha, md.mde_concepto AS concepto, md.mde_monto AS monto, m.mov_encargado as encargado, m.mov_obs AS observaciones, m.mov_tipo as tipo, md.mde_id as id " +
                                "FROM movimientos AS m, movimientosdetalles AS md " +
                                "WHERE m.mov_id = md.mov_id AND md.cue_nombre LIKE  '{1}' AND m.timestamp BETWEEN '{1}' AND '{2}')" +
                                "ORDER BY fecha", cuenta, fechadesde, fechahasta)
