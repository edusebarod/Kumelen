Public Class visRepAlumnosXCursos
    ...
    Private Sub visRepAlumnosXCursos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim SQL As String = String.Format("SELECT c.cur_nombre as Sala, CONCAT(p.per_apellido, ', ', p.per_nombre) as Alumno, p.per_dni AS DNI, a.alu_descuento AS Beca FROM cursos as c, personas as p, alumnos as a WHERE c.cur_id = {0} AND a.per_id = p.per_id AND a.alu_curso = c.cur_id AND a.alu_turno LIKE '{1}' AND a.alu_activo = 1 ORDER BY Alumno; SELECT cur_nombre FROM cursos WHERE cur_id = {2}", idCurso, turno, idCurso)
