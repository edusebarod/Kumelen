Public Class frmServicios2
    ...
    Private Sub cmdGrabar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdGrabar.Click
        If Validar() Then
            ...
            Serv.tipo = cbxTipo.SelectedIndex
            If txtCantidad.Text <> "" Then
                Serv.cantidad = Trim(txtCantidad.Text)
            Else
                Serv.cantidad = 0
            End If
            If Editar Then
                repo.Editar(Serv, False)
            Else
                repo.Agregar(Serv, False)
            End If
    ...
    Private Sub cmdBuscaNombre_Click(sender As Object, e As EventArgs) Handles cmdBuscaNombre.Click
        Serv.id = 0
        Dim CuerpoSelect As String = "serv_id, serv_codigo, serv_precio, serv_nombre, serv_tipo"
        FormatoBusqueda("servicios", CuerpoSelect, "serv_nombre", Trim(txtNombre.Text), False, "serv_id", "serv_nombre", Serv.id)
        frmBusqueda.ShowDialog()
        Serv.id = frmBusqueda.Codigo
        If Serv.id <> 0 Then
            Dim repo As New RepoServicio
            If repo.Traer_X_ID(Serv.id, Serv, False) Then
                AcomodoProducto()
    ...
    Private Sub cmdBuscaCod_Click(sender As Object, e As EventArgs) Handles cmdBuscaCod.Click
        Dim repo As New RepoServicio
        If repo.Traer_X_codigo(Trim(txtCodigo.Text), Serv, False) Then
            AcomodoProducto()
            AcomodoDespuesDeBusqueda()
        Else
    ...
    Private Sub AcomodoProducto()
        ...
        cbxTipo.SelectedIndex = Serv.tipo
        txtCantidad.Text = Serv.cantidad
    End Sub
